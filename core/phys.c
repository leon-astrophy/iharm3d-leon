...
// convert from primitive variables to flux/conservative variables, depends on the parameter dir
//////////////////////////////////////////////////////////////
...
#pragma omp for collapse(3)
    ...
    //contravariant magnetic 4-vector
#pragma omp for collapse(3)
    ZSLOOP(kstart, kstop, jstart, jstop, istart, istop) {
      bcon_calc(S, i, j, k);
      //lower_grid(S->bcon, S->bcov, G, i, j, k, loc);
  ...
  int loc, int dir, GridDouble cmax, GridDouble cmin)
{
  ...
  // initialize convariant component
  DLOOP1 {
    Acov[mu] = 0.;
  }
  Acov[dir] = 1.;
...
{
  // allocate arrays for wind source term
#if WIND_TERM
...
#pragma omp parallel for simd collapse(2)
  ZLOOP {
    ...
    /* temperature, in units of c^2, of new plasma */
    double Tp = 10. ; 
